...
There are two options available for running the medicine-chain sample
...
curl -s -X POST \
  ...
  -d '{
	"peers": ["peer0.org1.example.com"],
	"chaincodeName":"mycc",
	"chaincodePath":"github.com/medicine_chain_cc/go",
...
This invoke request is signed by peers from both orgs, *org1*, *org2* & *org3*.
```
curl -s -X POST \
